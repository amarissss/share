...
/*    Less    */
@fnord:  "Less is more";
@var:    "fnord";          // 将var替换成fnord，即@@var等价于@fnord
main::after {
  content: @@var;
}
...
```
#### Importing  
  1. 引入less文件，.less后缀可带可不带。
  2. 引入css文件且不想Less对它进行处理，只需要使用.css后缀就可以。  
  3. @import的位置可随意放置。
  4. 使用@import (reference)导入外部文件，只引用，不编译。  
```html
@import 'lib.less';
@import 'lib';
